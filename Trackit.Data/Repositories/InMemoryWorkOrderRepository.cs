...
{
    /// <summary>
    /// In-memory implementation of <see cref="IWorkOrderRepository"/> used by tests and CLI scenarios
    /// that do not require a persistent backing store.
    /// </summary>
    public sealed class InMemoryWorkOrderRepository : IWorkOrderRepository
    {
        ...
        public Task<int> AddAsync(WorkOrder wo, CancellationToken ct = default)
        {
            ...
            {
                var id = ++_nextId;
                var stored = wo with { Id = id };
                _byId[id] = Copy(stored);
